...
<b>Wordcloud - Vergleich (In der Verwendung um 90° zu drehen und die Labels zu entfernen</b>
```{r}
...
textplot_wordcloud(dfm_trim(comp_dfm, dmin_count = 5, verbose = FALSE), max_words = 300, min_size = 0.7, min_count = 5, labelcolor = "black", rotation = 1,
                   comparison = TRUE, color = c("darkblue", "darkgreen"))
```

<b>Statistics - Vergleich: leider keine Korrelation auffindbar</b>
```{r}
tweets_emo_avg <-
  tweets_clean_joined_emo %>%
  group_by(party) %>%
...
```
 ...
 <b>Violin Plots brauchen noch überarbeitung mit ggtext_repel und oder gg_label_repel</b>
```{r}
...
<b>Lineplots über Zeit - erratische Amplituden, aber keine nennenswerten Durschnittsunterschiede</b>
```{r}
...
<b>Deutschland - Valence</b>
```{r}
t.test(tweets_disc_afd$valence, tweets_disc_grde$valence, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```
<b>Deutschland - Dominance</b>
```{r}
t.test(tweets_disc_afd$dominance, tweets_disc_grde$dominance, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```

<b>Deutschland - Arousal</b>
```{r}
t.test(tweets_disc_afd$arousal, tweets_disc_grde$arousal, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```



<b>Österreich - Valence</b>
```{r}
t.test(tweets_disc_fpoe$valence, tweets_disc_groe$valence, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```
<b>Österreich - Dominance</b>
```{r}
t.test(tweets_disc_fpoe$dominance, tweets_disc_groe$dominance, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```

<b>Österreich - Arousal</b>
```{r}
t.test(tweets_disc_fpoe$arousal, tweets_disc_groe$arousal, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```
...
<b>Links/Rechts - Valence</b>
```{r}
t.test(tweets_right$valence, tweets_left$valence, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```
<b>Links/Rechts - Dominance</b>
```{r}
t.test(tweets_right$dominance, tweets_left$dominance, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```

<b>Links/Rechts - Arousal</b>
```{r}
t.test(tweets_right$arousal, tweets_left$arousal, var.equal = FALSE, alternative = "two.sided", conf.level = 0.95)
```