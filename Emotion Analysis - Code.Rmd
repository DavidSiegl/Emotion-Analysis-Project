...
# }
# 
# write.csv(df, "./data/clean/tweets_words.csv")
```


```{r}
tweets_words <- read.csv("./data/clean/tweets_words.csv")

join_dict <- function(words, dict){
  tf <- words
...
score <- join_dict(tweets_words, dict)
```


```{r}
tweets_clean_joined_ABaerbock <- tweets_clean_joined %>%
  filter(screen_name == "ABaerbock")

texts <- paste(tweets_clean_joined$text, tweets_clean_joined$text, sep = " ")
df <- data.frame(words = character(0), frequency = numeric(0))
for(i in 1:nrow(tweets_clean_joined_ABaerbock)){
  text <- texts[i]
  words <- tweets_words(text)
  df <- rbind(df, words)
}

score <- join_dict(df, dict)
print(score)
```
