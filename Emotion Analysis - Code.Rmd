...
df_comp <- rbind(df_comp, paste_left, paste_right)
df_comp <- cbind(df_comp, left_right)
colnames(df_comp) <- c("text", "left_right")
rownames(df_comp) <- c("left", "right")

corp_comp <- corpus(df_comp, text_field = "text")

comp_dfm <- get_dfm(corp_comp)

textplot_wordcloud(dfm_trim(comp_dfm, dmin_count = 5, verbose = FALSE), max_words = 300, min_size = 0.7, min_count = 5, labelcolor = "black", rotation = 0,
                   comparison = TRUE, color = c("darkgreen", "darkblue"))
```
```{r}
tweets_emo_avg <-
  tweets_clean_joined_emo %>%
